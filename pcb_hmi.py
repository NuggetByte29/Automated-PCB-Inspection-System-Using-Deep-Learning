...
class PCBInspectionHMI(ctk.CTk):
    ...
    def _build_chart(self, parent):
        ...
        ctk.CTkButton(hdr, text="↺  Refresh", width=90, height=28,
                      ...
                      font=ctk.CTkFont(size=11),
                      corner_radius=6, command=self._reset_all
                      ).pack(side="right")
    ...
    def _reset_all(self):
        """Reset all stats, log, chart and state back to zero."""
        # Reset counters
        self.total_inspected   = 0
        self.total_pass        = 0
        self.total_reject      = 0
        ...
        # Reset stat cards
        self.total_val.configure(text="0")
        self.pass_val.configure(text="0")
        ...
        self._set_status("●  System reset — Waiting for next PCB…", "#33cc66", "#112200")
    ...
    def _build_esp32_panel(self, parent):
        ...
        ctk.CTkLabel(outer, text="Arduino  ·  CONVEYOR CONTROL",
                     font=ctk.CTkFont(size=12, weight="bold"),
                     text_color="#445566").pack(anchor="w", padx=12, pady=(10, 6))

        # Cooldown setting row
        cd_row = ctk.CTkFrame(outer, fg_color="#13131e")
        cd_row.pack(fill="x", padx=12, pady=(0, 6))
        ctk.CTkLabel(cd_row, text="Scan cooldown:",
                     font=ctk.CTkFont(size=11), text_color="#667788").pack(side="left")
        self.cooldown_label = ctk.CTkLabel(
            cd_row, text=f"{self.INSPECTION_COOLDOWN:.1f}s",
            font=ctk.CTkFont(size=11, weight="bold"), text_color="#00ccff")
        self.cooldown_label.pack(side="right")
        self.cooldown_slider = ctk.CTkSlider(
            cd_row, from_=1.0, to=10.0, number_of_steps=18,
            width=140, command=self._on_cooldown_change)
        self.cooldown_slider.set(self.INSPECTION_COOLDOWN)
        self.cooldown_slider.pack(side="right", padx=(8, 8))
    ...
    def _save_pdf(self):
        ...
        try:
            ...
            if snap_path:
                ...
                snap_title = Paragraph("Defect Snapshot (Last Capture)",
                                       ParagraphStyle("h2", fontSize=13,
                                                      fontName="Helvetica-Bold",
                                                      textColor=colors.HexColor("#003366"),
                                                      spaceAfter=6))
                snap_img = RLImage(snap_path, width=target_w, height=target_h)
                # KeepTogether keeps title and image on the same page
            ...
            if self.full_log:
                ...
                log_data = [["#", "Timestamp", "Defect Class",
                              "Confidence", "Status"]]
                for entry in self.full_log:   # max 50 rows in PDF
                    log_data.append([
    ...
    def start_camera(self):
        ...
        # Start serial reader FIRST before sending START
        ...
        self.thread.start()
        self._set_status("●  Waiting for next PCB…", "#33cc66", "#112200")
    ...
    def _camera_loop(self):
        ...
        while self.camera_running:
            ...
            # PCB present — run AI and show boxes on live feed continuously
            ...
            # Record inspection result only once (first frame)
            if not self._inspection_locked:
                # Auto mode: freeze annotated frame automatically
                if self.auto_mode:
            ...
            # Manual freeze — user presses Freeze/Capture button
            if self._trigger_freeze:
                self._trigger_freeze = False
                self.frozen_frame = annotated.copy()
                self.after(0, self._update_frozen_panel)
    ...
    def _serial_reader(self):
        """Background thread: reads messages from Arduino and updates pcb_present."""
        ...
        while self.camera_running:
            ...
            try:
                if self.serial_conn.in_waiting:
                    ...
                    print(f"[SERIAL RX] '{line}'")   # debug: print every message
                    if line == "PCB_DETECTED":
                        print("[SERIAL] PCB_DETECTED → waiting 0.5s for PCB to settle")
                        self.after(0, self._set_status,
                                   "⬡  PCB detected — stabilising…",
                                   "#ffaa00", "#1a1200")
                        # Wait 0.5s for conveyor to fully stop before AI starts
                        time.sleep(1.0)
                        self.pcb_present = True
                        print("[SERIAL] AI activated")
                        self.after(0, self._set_status,
                                   "⬡  Inspecting PCB…",
                                   "#00ccff", "#001a2a")
            except Exception as e:
    ...
    def _update_stats(self, defect_count, elapsed_ms, detections):
        """Called once per inspection (when pcb_present=True and not locked)."""
        self.fps_val.configure(text=str(elapsed_ms))

        # Lock immediately — prevent re-recording, but keep pcb_present=True
        # so AI keeps running on live feed until cooldown expires
        self._inspection_locked = True
        ...
        # Start the independent cooldown countdown
        self._start_cooldown(defect_count)

    def _start_cooldown(self, defect_count):
        """Tick the cooldown timer every 100ms using self.after — independent of camera loop."""
        total_ms   = int(self.INSPECTION_COOLDOWN * 1000)
        interval   = 100   # tick every 100ms
        ticks_left = [total_ms // interval]

        if defect_count > 0:
            base_txt   = f"✖  REJECT  —  {defect_count} defect{'s' if defect_count>1 else ''} found!"
            txt_color  = "#ff8888"
            bg_color   = "#4a0808"
        else:
            base_txt   = "✔  PASS  —  No defects detected"
            txt_color  = "#88cc88"
            bg_color   = "#0a1f0a"

        def tick():
            remaining = ticks_left[0] * interval / 1000
            self._set_status(f"{base_txt}  (next scan in {remaining:.1f}s)",
                             txt_color, bg_color)
            ticks_left[0] -= 1
            if ticks_left[0] > 0:
                self.after(interval, tick)
            else:
                # Cooldown done — turn off AI, resume conveyor
                self.pcb_present = False        # AI off immediately
                self._send_serial("SENSOR_BYPASS\n")
                self._send_serial("START\n")
                self._set_status("●  Waiting for next PCB…", "#33cc66", "#112200")

                # Keep inspection locked for BYPASS_DURATION (3s) so the
                # clearing PCB cannot trigger a second inspection mid-movement
                BYPASS_DURATION = 3.0
                def _unlock_after_bypass():
                    self._inspection_locked = False
                self.after(int(BYPASS_DURATION * 1000), _unlock_after_bypass)
