...
class PCBInspectionHMI(ctk.CTk):
    ...
    def __init__(self):
        ...
        # ── Inspection gating ─────────────────────────────────────────────
        # Prevents the same PCB from being counted hundreds of times.
        # After one inspection result is logged, the system waits
        # INSPECTION_COOLDOWN seconds before logging the next one.
        self.INSPECTION_COOLDOWN = 3.0   # seconds — adjust to your conveyor speed
        self._last_inspection_time = 0
        self._inspection_locked    = False   # True while in cooldown
    ...
    def _update_stats(self, defect_count, elapsed_ms, detections):
        # Always update inference speed display
        self.fps_val.configure(text=str(elapsed_ms))

        now = time.time()

        # ── Cooldown gate ─────────────────────────────────────────────────
        # If still within cooldown window, just update the live status
        # display but do NOT count or log anything.
        if self._inspection_locked:
            remaining = self.INSPECTION_COOLDOWN - (now - self._last_inspection_time)
            if remaining > 0:
                if defect_count > 0:
                    self._set_status(
                        f"✖  REJECT  —  {defect_count} defect"
                        f"{'s' if defect_count > 1 else ''} found!  "
                        f"(next scan in {remaining:.1f}s)",
                        "#ff8888", "#4a0808"
                    )
                else:
                    self._set_status(
                        f"✔  PASS  —  Next scan in {remaining:.1f}s",
                        "#88cc88", "#0a1f0a"
                    )
                return   # ← skip counting entirely
            else:
        ...
        # Log this single inspection event
